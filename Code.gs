// ============================================================
//  ONE LINE BL TRACKER — Google Apps Script Backend  v3.0
//  APIs reverse-engineered from live browser session — EXACT
// ============================================================
...
function trackSingleBL(blNumber, notes) {
  ...
  const record = {
    blNumber: blNumber.toUpperCase(), searchCode: cleanBL,
    containerNo: live.containerNo || '', type: live.type || '',
    origin: live.origin || '', destination: live.destination || '',
    vessel: live.vessel || '', voyage: live.voyage || '',
    eta: live.eta || '', status: live.status || 'Pending',
    lastUpdated: new Date().toISOString(), notes: notes, trackUrl: trackUrl
  };
  writeToSheet(record);
  return record;
}
...
function fetchONELineData(cleanBL) {
  const empty = { containerNo:'', type:'', origin:'', destination:'', vessel:'', voyage:'', eta:'', status:'Pending' };
  const headers = {
    'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) Chrome/124.0.0.0 Safari/537.36',
    'Accept': 'application/json', 'Referer': ONE_BASE + '/one-ecom/manage-shipment/cargo-tracking'
  };
  const result = {};
  try {
    const r1 = UrlFetchApp.fetch(ONE_BASE + '/api/v1/edh/vessel/track-and-trace/voyage-list?booking_no=' + cleanBL, { method:'GET', headers:headers, muteHttpExceptions:true });
  ...
  try {
    const r2 = UrlFetchApp.fetch(ONE_BASE + '/api/v1/edh/containers/track-and-trace/cop-events?booking_no=' + cleanBL, { method:'GET', headers:headers, muteHttpExceptions:true });
...
function ensureHeaders(sheet) {
  if (!sheet.getRange(1,1).getValue()) {
    const h = ['BL Number','Search Code','Container No','Type','Origin','Destination','Vessel','Voyage','ETA','Status','Last Updated','Notes','Track URL'];
...
function writeToSheet(rec) {
  const sheet = getSheet(); ensureHeaders(sheet);
  const row = [rec.blNumber,rec.searchCode,rec.containerNo,rec.type,rec.origin,rec.destination,rec.vessel,rec.voyage,rec.eta,rec.status,rec.lastUpdated,rec.notes,rec.trackUrl];
...
function getSheetData() {
  const sheet = getSheet(); const last = sheet.getLastRow();
  if (last < 2) return [];
  return sheet.getRange(2,1,last-1,13).getValues().filter(r=>r[0]).map(r=>({
    blNumber:r[0]||'',origin:r[4]||'',destination:r[5]||'',
    vessel:r[6]||'',voyage:r[7]||'',eta:r[8]||'',status:r[9]||'Pending',
    lastUpdated:r[10]||'',notes:rs[11]||'',trackUrl:r[12]||''
  }));
}
...
function deleteFromSheet(blNumber) {
  ...
  for (let i=col.length-1; i>=0; i--) {
    if (String(col[i][0]).toUpperCase() === blNumber.toUpperCase()) { sheet.deleteRow(i+2); return { deleted:true }; }
  }
  return { deleted:false };
}

function refreshAllBLs() {
  const records = getSheetData(); const updated = [];
  for (const rec of records) {
    try {
      const cleanBL = rec.blNumber.replace(/^ONEY/i,'').trim().toUpperCase();
      const live = fetchONELineData(cleanBL);
      const fresh = { ...rec, ...live, lastUpdated:new Date().toISOString() };
      writeToSheet(fresh); updated.push(fresh); Utilities.sleep(800);
    } catch (e) { updated.push(rec); }
  }
  return updated;
}

function setupAutoRefresh() {
  ScriptApp.getProjectTriggers().filter(t=>t.getHandlerFunction()==='scheduledRefresh').forEach(t=>ScriptApp.deleteTrigger(t));
  ScriptApp.newTrigger('scheduledRefresh').timeBased().everyHours(6).create();
  return { ok:true };
}

function scheduledRefresh() {
  try { refreshAllBLs(); } catch (e) { Logger.log(e.message); }
}
