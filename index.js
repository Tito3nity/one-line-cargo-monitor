// server/index.js
// ONE Line BL Tracker — Render Web Service (Express)
// Repackaged from netlify/functions/track.js v6.0 — identical tracking logic.
// Exposes:  GET /api/track?action=track&bl=BLNUMBER
//           GET /api/track?action=ping   → health check
//           GET /            → simple health page (keeps cold-start visible)
//
// Why Render and not GitHub Pages: ONE Line blocks cross-origin browser
// requests (CORS) and filters by server IP. The spoofed Origin/Referer
// headers below only work from a server. GitHub Pages is static-only, so
...
// Permissive CORS so the GitHub Pages page (any origin) can call this service.
...
app.get('/api/track', async (req, res) => {
  ...
  try {
    ...
    const data    = await fetchONELineData(cleanBL, req.query.notes || '');
    res.json(data);
  } catch (err) {
    // Mirror original behaviour: 200 + error field so the UI keeps the row as Pending.
    res.json({ error: err.message, status: 'Pending', blNumber: bl });
  }
});
...
app.listen(PORT, () => console.log('ONE Line Tracker API listening on port ' + PORT));
